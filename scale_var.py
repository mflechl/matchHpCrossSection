...
def create_scale_var_tables(m_Hs):
	m_Hdata = {}
...
#	print ""
	print ""
	print "Creating tables for scale_var from %d jobs." % (jobs) 
...
#		print "Processing job_%s/job_scale_var%d" % (cm_energy, i)
		f = open("jobs_%s/job_scale_var%d" % (cm_energy, i), "r")
	...
	#	mass.sort(cmp = lambda x,y: x[0] < y[0])
		...
		f = open("tables_%s/scale_var_%dGeV" % (cm_energy, mass_name), "w")
...
def get_scale_var(m_H):
	data = read_scale_var_table("scale_var_%dGeV" % m_H)
...
def scale_var_analysis(m_Hs, latex_res):
	...
	for m_H in m_Hs: 
		data = read_scale_var_table("scale_var_%dGeV" % m_H[0])
		
		scale_var = get_scale_var(m_H[0])

		print " %4d      +%.2e         -%.2e             +%.2e         -%.2e   %.2e" % (m_H[0], scale_var[5], scale_var[4], scale_var[2], scale_var[1], scale_var[0])
		if latex_res:
			latex_res.write(r" %s & %.2e & +%.2e & -%.2e & %.2e & +%.2e & -%.2e \\" % (m_H[0], scale_var[3], scale_var[5], scale_var[4], scale_var[0], scale_var[2], scale_var[1]) + "\n")
			latex_res.write(r"\hline")


		data_x.append(m_H[0])
		data_nlo.append((1.0, scale_var[2]/scale_var[0], scale_var[1]/scale_var[0]))
		data_lo.append((1.0, scale_var[5]/scale_var[3], scale_var[4]/scale_var[3]))
